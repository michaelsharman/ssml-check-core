...
const check_amazon_auto_breaths = (parent, index, errors, element, platform, locale) => {
    ...
    attributes.forEach(attribute => {
        if (attribute === 'duration') {
            if (['default', 'x-short', 'short', 'medium', 'long', 'x-long'].indexOf(element.attributes.duration) === -1) {
                errors.push(createTagError(element, attribute));
                element.attributes.duration = 'medium';
            }
        } else if (attribute === 'volume') {
            if (['default', 'x-soft', 'soft', 'medium', 'loud', 'x-loud'].indexOf(element.attributes.volume) === -1) {
                errors.push(createTagError(element, attribute));
                element.attributes.volume = 'medium';
            }
        } else if (attribute === 'frequency') {
            if (['default', 'x-low', 'low', 'medium', 'high', 'x-high'].indexOf(element.attributes.frequency) === -1) {
                errors.push(createTagError(element, attribute));
...
const check_amazon_domain = (parent, index, errors, element, platorm, locale) => {
    ...
    attributes.forEach(attribute => {
        if (attribute === 'name') {
            ...
            if (!allowedValues[element.attributes.name] || (locale && allowedValues[element.attributes.name].indexOf(locale) === -1)) {
                errors.push(createTagError(element, attribute));
    ...
    attributes.forEach(attribute => {
        if (platform === 'google' && attribute === 'clipBegin') {
            ...
            if (!element.attributes.soundLevel.match(/^[+-][0-9]+(\.[0-9]+)?dB$/g) || !soundLevel.inRange) {
                errors.push(createTagError(element, attribute));
                element.attributes.soundLevel = soundLevel.value < 0 ? soundLevel.value + 'dB' : '+' + soundLevel.value + 'dB';
            }
...
const check_break = (parent, index, errors, element, platform, locale) => {
    ...
    attributes.forEach(attribute => {
        if (attribute === 'strength') {
            if (['none', 'x-weak', 'weak', 'medium', 'strong', 'x-strong'].indexOf(element.attributes.strength) === -1) {
                errors.push(createTagError(element, attribute));
...
const check_emphasis = (parent, index, errors, element, platform, locale) => {
    ...
    attributes.forEach(attribute => {
        if (attribute === 'level') {
            if (['strong', 'moderate', 'reduced'].indexOf(element.attributes.level) === -1) {
...
const check_media = (parent, index, errors, element, platform, locale) => {
    ...
    attributes.forEach(attribute => {
        if (attribute === 'xml:id') {
            ...
            if (
                !element.attributes.begin.match(/^[+-]?[0-9]+(\.[0-9]+)?(h|min|s|ms)$/g) &&
            ...
            if (
                !element.attributes.end.match(/^[+-]?[0-9]+(\.[0-9]+)?(h|min|s|ms)$/g) &&
...
const check_phoneme = (parent, index, errors, element, platform, locale) => {
    ...
    attributes.forEach(attribute => {
        if (attribute === 'alphabet') {
            if (['ipa', 'x-sampa'].indexOf(element.attributes.alphabet) === -1) {
                errors.push(createTagError(element, attribute));
        ...
        if (attribute === 'rate') {
            ...
            if (['x-low', 'low', 'medium', 'high', 'x-high'].indexOf(element.attributes.pitch) === -1) {
                ...
                if (!element.attributes.pitch.match(/^[+-][0-9]+(\.[0-9]+)?%$/g) || !pitch.inRange) {
                    ...
                    if (platform !== 'google' || !element.attributes.pitch.match(/^[+-]+[0-9]+(\.[0-9]+)?st$/g)) {
                        errors.push(createTagError(element, attribute));
                        element.attributes.pitch = pitch.value < 0 ? pitch.value + '%' : '+' + pitch.value + '%';
                    }
                }
            }
        } else if (attribute === 'volume') {
    ...
    attributes.forEach(attribute => {
        if (attribute === 'interpret-as') {
            if (
                ...
                let supported = false;
                if (platform === 'amazon' && ['number', 'digits', 'address', 'interjection'].indexOf(element.attributes['interpret-as'] !== -1)) {
                    supported = true;
                } else if (platform === 'google' && ['bleep', 'verbatim'].indexOf(element.attributes['interpret-as'] !== -1)) {
                    supported = true;
                }
            ...
            if (isDate) {
                if (['mdy', 'dmy', 'ymd', 'md', 'dm', 'ym', 'my', 'd', 'm', 'y'].indexOf(element.attributes.format) === -1) {
                    errors.push(createTagError(element, attribute));
...
const check_w = (parent, index, errors, element, platform, locale) => {
    ...
    attributes.forEach(attribute => {
        if (attribute === 'role') {
            if (['amazon:VB', 'amazon:VBD', 'amazon:NN', 'amazon:DT', 'amazon:IN', 'amazon:JJ', 'amazon:SENSE_1'].indexOf(element.attributes.role) === -1) {
                errors.push(createTagError(element, attribute));
