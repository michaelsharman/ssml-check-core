...
function checkForValidTagsRecursive(parent, index, errors, element, platform, locale, unsupportedTags) {
    ...
    if (element.name) {
        ...
        const validTags = ['audio', 'break', 'emphasis', 'lang', 'mark', 'p', 'phoneme', 'prosody', 's', 'say-as', 'speak', 'sub'].filter(
            t => !unsupportedTags.includes(t)
        );
        const validAmazonTags = ['amazon:auto-breaths', 'amazon:breath', 'amazon:effect', 'amazon:emotion', 'amazon:domain', 'voice', 'w', 'alexa:name'].filter(
            t => !unsupportedTags.includes(t)
        );
        const validGoogleTags = ['par', 'seq', 'media', 'desc', 'voice'].filter(t => !unsupportedTags.includes(t));

        if (
            validTags.indexOf(element.name) === -1 &&
            !(
...
function checkInternal(ssml, options, fix) {
    ...
    try {
        ...
        if (!result.elements || result.elements.length !== 1 || result.elements[0].name !== 'speak') {
...
module.exports = {
    check: function (ssml, options) {
        return checkInternal(ssml, options).then(result => {
            return result.errors;
        });
    },
    verifyAndFix: function (ssml, options) {
        return checkInternal(ssml, options, true).then(result => {
            let ssml;
